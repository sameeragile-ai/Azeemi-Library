...
const CATEGORIES = ['All', 'Philosophy', 'Metaphysics', 'Healing', 'Practice', 'Biography', 'Poetry', 'Science'];
const ITEMS_PER_PAGE = 12;

export default function LibraryBrowser({ onSelectBook, state, onStateChange }: Props) {
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          <h2 className="text-4xl font-bold text-text-main tracking-tight">Spiritual Library</h2>
          <p className="text-text-muted">Explore over {books.length} volumes of digitized wisdom.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          <div className="relative flex-1 sm:w-80">
            ...
            {state.searchQuery && (
              <button 
                onClick={() => onStateChange({ ...state, searchQuery: '', page: 1 })}
      ...
      <div className="flex items-center justify-between text-sm text-text-muted">
        <p>Showing {paginatedBooks.length} of {filteredBooks.length} results</p>
        {state.searchQuery && (
          <p>Search results for "<span className="text-text-main font-bold">{state.searchQuery}</span>"</p>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        {paginatedBooks.length > 0 ? (
              ...
              viewMode === 'grid' ? (
                ...
                >
                  ...
                  <div className="p-6 space-y-2">
                    <h4 className="text-lg font-bold text-text-main group-hover:text-primary transition-colors line-clamp-1">{book.title}</h4>
                    <p className="text-sm text-text-muted font-medium">{book.author}</p>
                    <div className="flex flex-wrap gap-1 pt-2">
                      {book.tags.slice(0, 2).map(tag => (
                        <span key={tag} className="text-[10px] text-text-muted bg-gray-100 px-2 py-0.5 rounded">#{tag}</span>
                      ))}
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-bold text-primary bg-primary/10 px-3 py-1 rounded-full">{book.category}</span>
                    </div>
                    <p className="text-sm text-text-muted font-medium">{book.author}</p>
                    <p className="text-sm text-text-muted line-clamp-1 mt-2">{book.description}</p>
                  </div>
          ...
          <div className="text-center py-24 space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-bold text-text-main">No books found</h3>
              <p className="text-text-muted">Try adjusting your search or category filters.</p>
            </div>
            <button 
              onClick={() => onStateChange({ page: 1, searchQuery: '', category: 'All' })}
